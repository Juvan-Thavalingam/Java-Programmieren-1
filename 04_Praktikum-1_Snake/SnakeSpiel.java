...
public class SnakeSpiel {
  ...
  private List<Point> goldstueckeListe = new ArrayList<>();
  ...
  private void spielInitialisieren() {
    ...
    setAnzahlGoldstuecke(10);
    schlange = new Schlange(30, 2);
  }

  private void setAnzahlGoldstuecke(int anzahl){
    for(int i = 0; i < anzahl; i++){
      goldStueck = new Point(spielfeld.erzeugeZufallspunktInnerhalb());
      goldstueckeListe.add(goldStueck);
    }
  }
  ...
  private void zeichneSpielfeld() {
    ...
    for (int y = 0; y < spielfeld.gibHoehe(); y++) {
      for (int x = 0; x < spielfeld.gibBreite(); x++) {
        ...
        for (int i = 0; i < goldstueckeListe.size(); i++){
          if (goldstueckeListe.get(i).equals(punkt)){
            ausgabeZeichen = '$';
          }
        }
  ...
  private void ueberpruefeSpielstatus() {
    for (int i = 0; i < goldstueckeListe.size(); i++){
      if(goldstueckeListe.get(i).equals(schlange.gibPosition())){
        goldstueckeListe.remove(i);
        schlange.wachsen();
